...
template <class T, unsigned N> cgv::math::fvec<T, N - 1> w_clip(const cgv::math::fvec<T, N>& v)
{
...
template <class T> T znear_from_mat(const cgv::math::fmat<T, 4, 4>& projection_matrix)
{
	static cgv::math::fvec<T, 3> p_near(0, 0, -1);
...
/// NOTE: not optimized - calculates the inverse of the input matrix, could be done in different more efficient ways
template <class T> T zfar_from_mat(const cgv::math::fmat<T, 4, 4>& projection_matrix)
{
...
template <class T>
const std::pair<const T, const T> project_box_onto_dir(const cgv::media::axis_aligned_box<T, 3>& box,
													   const cgv::math::fvec<T, 3> d)
{
