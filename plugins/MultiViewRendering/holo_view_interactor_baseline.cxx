...
void holo_view_interactor_baseline::post_process_surface(cgv::render::context& ctx)
{
	...
	/* quilt_nr_cols = 5;
	quilt_nr_rows = 9;
	if (!warp_fbo.is_created() || warp_fbo.get_width() != quilt_width || warp_fbo.get_height() != quilt_height) {
				...
				for (quilt_row = 0; quilt_row < quilt_nr_rows; ++quilt_row) {
					for (quilt_col = 0; quilt_col < quilt_nr_cols; ++quilt_col) {
						...
						for (int i = 0; i < nr_render_views; i++) {
							...
							baseline_shader.set_uniform(ctx, "epsilon", 0.02);
							baseline_shader.set_uniform(ctx, "x_start", i * view_width / 3.0);
							baseline_shader.set_uniform(ctx, "y_start", i * view_height / 1.0);
							baseline_shader.set_uniform(ctx, "x_divide", 3.0);
							baseline_shader.set_uniform(ctx, "y_divide", 1.0);
