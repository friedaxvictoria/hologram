/*#include "holo_view_interactor_baseline.h"
...
cgv::reflect::enum_reflection_traits<holo_view_interactor_baseline::MultiplexMode>
get_reflection_traits(const holo_view_interactor_baseline::MultiplexMode&)
{
	return cgv::reflect::enum_reflection_traits<holo_view_interactor_baseline::MultiplexMode>("single,quilt,volume");
}

void holo_view_interactor_baseline::set_default_values()
{
...
void holo_view_interactor_baseline::check_emulation_active()
{
...
void holo_view_interactor_baseline::plus_key_action(int i, cgv::gui::KeyAction action)
{
	switch (action) {
...
void holo_view_interactor_baseline::minus_key_action(int i, cgv::gui::KeyAction action)
{
	switch (action) {
	case cgv::gui::KA_RELEASE:
...
void holo_view_interactor_baseline::timer_event(double t, double dt)
{
	if (emulation_active) {
		unsigned i;
		// update key values
...
///
holo_view_interactor_baseline::holo_view_interactor_baseline(const char* name)
	: node(name), quilt_depth_buffer("[D]"), volume_depth_buffer("[D]")
{
	...
	connect(cgv::gui::get_animation_trigger().shoot, this, &holo_view_interactor_baseline::timer_event);
...
std::string holo_view_interactor_baseline::get_type_name() const { return "holo_view_interactor_baseline"; }

/// overload to stream help information to the given output stream
void holo_view_interactor_baseline::stream_help(std::ostream& os)
{
...
void holo_view_interactor_baseline::stream_stats(std::ostream& os)
{
	os << "holo_view_interactor_baseline:\n\a";
...
unsigned holo_view_interactor_baseline::get_viewport_index(unsigned col_index, unsigned row_index) const
{
...
void holo_view_interactor_baseline::enable_viewport_splitting(unsigned nr_cols, unsigned nr_rows)
{
...
/// pointers are passed
bool holo_view_interactor_baseline::is_viewport_splitting_enabled(unsigned* nr_cols_ptr, unsigned* nr_rows_ptr) const
{
...
void holo_view_interactor_baseline::disable_viewport_splitting() { do_viewport_splitting = false; }

holo_view_interactor_baseline::ivec4 holo_view_interactor_baseline::split_viewport(const ivec4 vp, int col_idx,
																		  int row_idx) const
{
	ivec4 new_vp;
...
void holo_view_interactor_baseline::activate_split_viewport(cgv::render::context& ctx, unsigned col_index,
															unsigned row_index)
{
	if (!do_viewport_splitting)
		return;
...
void holo_view_interactor_baseline::deactivate_split_viewport(cgv::render::context& ctx)
{
	if (!do_viewport_splitting)
		return;
...
void holo_view_interactor_baseline::enable_viewport_individual_view(unsigned col_index, unsigned row_index, bool enable)
{
	unsigned view_index = get_viewport_index(col_index, row_index);
	ensure_viewport_view_number(view_index + 1);
...
cgv::render::view& holo_view_interactor_baseline::ref_viewport_view(unsigned col_index, unsigned row_index)
{
	unsigned view_index = get_viewport_index(col_index, row_index);
	ensure_viewport_view_number(view_index + 1);
...
int holo_view_interactor_baseline::get_modelview_projection_window_matrices(
...
void holo_view_interactor_baseline::get_vp_col_and_row_indices(cgv::render::context& ctx, int x, int y, int& vp_col_idx,
													  int& vp_row_idx)
{
...
double holo_view_interactor_baseline::get_z_and_unproject(cgv::render::context& ctx, int x, int y, dvec3& p)
{
	const dmat4 *MPW_ptr, *MPW_other_ptr;
...
void holo_view_interactor_baseline::set_view_orientation(const std::string& axes)
{
	dvec3 axis;
	double angle;
...
bool holo_view_interactor_baseline::handle(event& e)
{
		...
		if (ke.get_action() != KA_RELEASE) {
			...
			case KEY_F4:
				if (ke.get_modifiers() == 0) {
					render_mpx_mode = MultiplexMode(((int&)render_mpx_mode + 1) % 3);
...
void holo_view_interactor_baseline::on_rotation_change()
{
...
void holo_view_interactor_baseline::finish_frame(cgv::render::context& ctx)
{
...
void holo_view_interactor_baseline::gl_set_projection_matrix(cgv::render::context& ctx, float e, double aspect)
{
	dmat4 P;
	if (y_view_angle <= 0.1)
...
void holo_view_interactor_baseline::gl_set_modelview_matrix(cgv::render::context& ctx, float e, double aspect,
...
void holo_view_interactor_baseline::ensure_viewport_view_number(unsigned nr)
{
	if (views.size() < nr) {
...
bool holo_view_interactor_baseline::init(cgv::render::context& ctx)
{
...
void holo_view_interactor_baseline::init_frame(context& ctx)
{
	...
	// - tesselate
	heightmap = tessellator::quad(ctx, baseline_shader, {-half_aspect, -.5f, .0f},
										{half_aspect, .5f, .0f}, float(ctx.get_width()), float(ctx.get_height()));
	...
	case HM_VOLUME:
		...
		if (initiate_render_pass_recursion(ctx)) {
			...
			if (render_mpx_mode == HM_QUILT) {
				...
				for (vi = 0; vi < nr_render_views; ++vi) {
	...
	const float cam_depth_world = dot(cam_pos, cam_dir), heightmap_depth_eye = (float)z_far_derived - cam_depth_world,
				y_ext = (float)get_y_extent_at_depth(heightmap_depth_eye, false), x_ext = y_ext * aspect;
	...
	if (rpf & RPF_SET_PROJECTION) {
		gl_set_projection_matrix(ctx, current_e, aspect);
		inv_mat_proj_render[view_index] = inv(ctx.get_projection_matrix());
	}		
	...
	projection_dir[view_index] =
		  normalize(vec3(0.5f * current_e * get_eye_distance(), 0, -get_depth_of_focus()));
}

/// this method is called in one pass over all drawables after finish frame
void holo_view_interactor_baseline::after_finish(cgv::render::context& ctx)
{
	if (render_mpx_mode != HM_SINGLE && !multi_pass_ignore_finish(ctx) && multi_pass_terminate(ctx)) {
		disable_surface(ctx);
...
void holo_view_interactor_baseline::enable_surface(cgv::render::context& ctx)
{
	if (render_mpx_mode == HM_QUILT) {
		...
		if (!quilt_fbo.is_created() || quilt_fbo.get_width() != quilt_width || quilt_fbo.get_height() != quilt_height) {
			...
			quilt_fbo.attach(ctx, quilt_depth_tex, 1);
			...
			volume_fbo.get_height() != view_height || volume_render_tex.get_depth() != nr_render_views)
		{
			...
			volume_fbo.attach(ctx, volume_depth_tex, 0, 0, 1);
...
void holo_view_interactor_baseline::disable_surface(cgv::render::context& ctx)
{
	if (render_mpx_mode == HM_QUILT) {
...
void holo_view_interactor_baseline::post_process_surface(cgv::render::context& ctx)
{
	/////////////////////////////////////////////
	/// --NOTE--
	...
	if (!warp_fbo.is_created() || warp_fbo.get_width() != quilt_width ||
		warp_fbo.get_height() != quilt_height)
	{
		quilt_holo_tex.destruct(ctx);
		warp_fbo.destruct(ctx);
		warp_fbo.create(ctx, quilt_width, quilt_height);
		quilt_holo_tex.create(ctx, cgv::render::TT_2D, quilt_width, quilt_height);
		warp_fbo.attach(ctx, quilt_holo_tex);
	}

	warp_fbo.enable(ctx);
	warp_fbo.push_viewport(ctx);

	for (int k = 0; k < nr_holo_views; k++) {
		for (int i = 0; i < nr_render_views; i++) {
			...
			if (holo_mpx_mode == HM_QUILT){
				...
				baseline_shader.set_uniform(ctx, "projection_dir", projection_dir[i]);
				baseline_shader.set_uniform(ctx, "epsilon", 0.07);
				...
				quilt_depth_tex.disable(ctx);
				quilt_render_tex.disable(ctx);
			}			
		}
	}

	warp_fbo.pop_viewport(ctx);
	warp_fbo.disable(ctx);
	...
	if (generate_hologram) {
		...
		if (holo_mpx_mode == HM_QUILT)
			...
			warp_fbo.blit_to(ctx, BTB_COLOR_BIT, true);
		else {
			volume_fbo.attach(ctx, volume_holo_tex, view_index, 0,
...
///
void holo_view_interactor_baseline::draw(cgv::render::context& ctx)
{
	if (show_focus) {
		if (is_viewport_splitting_enabled()) {
			for (unsigned c = 0; c < nr_viewport_columns; ++c) {
...
void holo_view_interactor_baseline::draw_focus()
{
	glLineWidth(1.0f);
...
std::string holo_view_interactor_baseline::get_menu_path() const { return "View/Stereo Interactor"; }

/// you must overload this for gui creation
void holo_view_interactor_baseline::create_gui()
{
	...
	if (begin_tree_node("Holo Parameters", render_mpx_mode, true)) {
		align("\a");
	...
	if (begin_tree_node("Current View", view::focus(0), true)) {
		...
		connect_copy(add_control("Fix View Up Dir", fix_view_up_dir, "check")->value_change,
					 rebind(this, &holo_view_interactor_baseline::on_rotation_change));
...
void holo_view_interactor_baseline::on_set(void* m)
{
...
void holo_view_interactor_baseline::set_z_near(double z)
{
	cgv::render::clipped_view::set_z_near(z);
	update_member(&z_near);
	post_redraw();
}
void holo_view_interactor_baseline::set_z_far(double z)
{
	cgv::render::clipped_view::set_z_far(z);
	update_member(&z_far);
	post_redraw();
}
void holo_view_interactor_baseline::set_default_view()
{
...
/// you must overload this for gui creation
bool holo_view_interactor_baseline::self_reflect(cgv::reflect::reflection_handler& srh)
{
...
cgv::base::object_registration_1<holo_view_interactor_baseline, const char*> obj1("Holo Interactor Baseline",
